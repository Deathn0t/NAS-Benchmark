...
def main():
    ...
    reg_crit = lambda xx: F.l1_loss(xx, target=torch.zeros_like(xx), reduction="sum")
    layers = 1
    n_nodes = 4
...
def train(
    ...
    for step, ((trn_X, trn_y), (val_X, val_y)) in enumerate(
        ...
        architect.unrolled_backward(trn_X, trn_y, val_X, val_y, lr, w_optim, reg_crit)
        alpha_optim.step()
        ...
        # reg_loss = 0
        # for alpha in model.alpha_normal:
        #     reg_loss += reg_crit(alpha)
        # reg_factor = 0.5
